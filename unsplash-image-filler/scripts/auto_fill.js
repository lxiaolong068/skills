...
async function scanAndFill() {
  ...
  const accessKey =
    argv.key || config.accessKey || process.env.UNSPLASH_ACCESS_KEY;
  ...
  if (!accessKey) {
    console.error(
      "Error: Unsplash Access Key is required. Provide it via --key, UNSPLASH_ACCESS_KEY env var, or unsplash.config.json",
    );
    process.exit(1);
  }
